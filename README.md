...
- `pip install -r requirements.txt` installs the validated dependency set. The train/eval scripts fail fast if the installed stack drifts.
- `scripts/prepare_data.py` writes canonical instruction rows with `prompt`, `response`, `text`, and preference rows with `prompt`, `chosen`, `rejected`.
- The commands below are smoke-sized. They validate the pipeline; they do not produce a high-quality assistant.
